#!/usr/bin/env python3
"""
asa.py — marketing-automation entry point.
...
def _config_dir() -> str:
    ...
    local = "./.fivetran/marketing-automation"
    if os.path.isdir(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".fivetran", "skills", "marketing-automation")


def _profile_path() -> str:
    if os.environ.get("MARKETING_AUTOMATION_PROFILE_PATH"):
        return os.environ["MARKETING_AUTOMATION_PROFILE_PATH"]
...
def cmd_resolve(family: str, refresh_on_miss: bool) -> int:
    ...
    if refresh_on_miss and _resolve_credentials():
        ...
        skill_id = raw.get("skill", {}).get("id", "marketing-automation")
        cmd_setup(
