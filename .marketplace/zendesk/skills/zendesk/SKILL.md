...
This skill uses a local profile at `~/.fivetran/skills/zendesk/profile.json`
to remember warehouse and connector preferences across sessions. First run creates it;
subsequent runs reuse it.

1. **Validate the local profile.**
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh validate
   ```
...
2. **First-run setup** (only when validate exits `60`, `61`, or `62`).
   ...
   **Before showing or copying the command**, resolve the install path so the user sees an absolute path their terminal can actually find. Run:
   ...
   - macOS: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh setup --skill zendesk" | pbcopy`
   - Windows: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh setup --skill zendesk" | clip`
   - Linux: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh setup --skill zendesk" | xclip -selection clipboard 2>/dev/null || echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh setup --skill zendesk" | xsel --clipboard 2>/dev/null`
   ...
   - `validate` still returns `60` → **silently run setup yourself** and present the result naturally:
     ```bash
     bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh setup --skill zendesk 2>&1; echo "EXIT:$?"
     ```
...
3. **Resolve connector context.**
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh resolve zendesk
   ```
...
5. **Refresh on relation-not-found.** If a query fails because a table or schema is missing, rerun resolve with refresh:
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh resolve zendesk --refresh-on-miss
   ```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh readiness
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh check-cli <bq|snowflake_cli|databricks_cli>
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/generate-dashboard.py \
...
| Error | Response |
|---|---|
| Warehouse connection failure | "Cannot connect to warehouse. Run `bash ${CLAUDE_PLUGIN_ROOT}/skills/zendesk/asa.sh check-cli <tool>` to diagnose auth." |
...
1. List tables in the resolved dataset: `bq ls {PROJECT_ID}:{SCHEMA}`
2. Inspect schema: `bq show --schema --format=prettyjson {PROJECT_ID}:{SCHEMA}.<table>`
3. Sample rows: `bq head -n 5 {PROJECT_ID}:{SCHEMA}.<table>`
