#!/usr/bin/env python3
"""
asa.py — sales-pipeline-analysis entry point.
...
def _config_dir() -> str:
    ...
    if os.path.isdir(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".fivetran", "skills", "sales-pipeline-analysis")


def _profile_path() -> str:
    if os.environ.get("SALES_PIPELINE_ANALYSIS_PROFILE_PATH"):
        return os.environ["SALES_PIPELINE_ANALYSIS_PROFILE_PATH"]
...
def _databricks_query(sql: str, timeout: int = 30) -> Optional[List]:
    ...
    # use the SQL Statement Execution REST API instead, via `databricks api post`
    # so we keep the CLI's existing auth profile.
    warehouse_id = os.environ.get("DATABRICKS_WAREHOUSE_ID", "").strip()
    if not warehouse_id:
        print("[asa] DATABRICKS_WAREHOUSE_ID not set. Set it to the id of a SQL "
              "warehouse in your Databricks workspace and re-run.", file=sys.stderr)
        return None
    body = json.dumps({
    ...
    if r.returncode != 0:
        print(f"[asa] warn: databricks query failed: {r.stderr.strip()}", file=sys.stderr)
        return None
    try:
        payload = json.loads(r.stdout)
    except Exception:
        return None
    if (payload.get("status") or {}).get("state") != "SUCCEEDED":
        err = (payload.get("status") or {}).get("error", {}).get("message", "")
        print(f"[asa] warn: databricks statement failed: {err}", file=sys.stderr)
        return None
...
def cmd_resolve(family: str, refresh_on_miss: bool) -> int:
    ...
    if refresh_on_miss and _resolve_credentials():
        ...
        skill_id = raw.get("skill", {}).get("id", "sales-pipeline-analysis")
        cmd_setup(
...
def main() -> int:
    ...
    if subcmd == "setup":
        ...
        skill_id = "sales-pipeline-analysis"
        refresh  = False
        i = 0
        while i < len(rest):
            arg = rest[i]
