...
def _config_dir() -> str:
    ...
    local = "./.fivetran/zendesk"
    if os.path.isdir(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".fivetran", "skills", "zendesk")


def _profile_path() -> str:
    if os.environ.get("ZENDESK_PROFILE_PATH"):
        return os.environ["ZENDESK_PROFILE_PATH"]
...
def cmd_resolve(family: str, refresh_on_miss: bool) -> int:
    ...
    if refresh_on_miss and _resolve_credentials():
        ...
        skill_id = raw.get("skill", {}).get("id", "zendesk")
        cmd_setup(
