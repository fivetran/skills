#!/usr/bin/env python3
"""
asa.py — unified store-performance-analysis entry point.
...
def _config_dir() -> str:
    ...
    if os.path.isdir(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".fivetran", "skills", "store-performance-analysis")


def _profile_path() -> str:
    if os.environ.get("STORE_PERFORMANCE_ANALYSIS_PROFILE_PATH"):
        return os.environ["STORE_PERFORMANCE_ANALYSIS_PROFILE_PATH"]
...
# in skills/ad-performance-analysis/asa.py and skills/sales-pipeline-analysis/asa.py.
...
def cmd_resolve(family: str, refresh_on_miss: bool) -> int:
    ...
    if refresh_on_miss and _resolve_credentials():
        ...
        skill_id = raw.get("skill", {}).get("id", "store-performance-analysis")
        cmd_setup(
...
def main() -> int:
    ...
    if subcmd == "setup":
        ...
        clear_skip = False
        skill_id = "store-performance-analysis"
        refresh  = False
        i = 0
        while i < len(rest):
            arg = rest[i]
