...
metadata:
  plugin: store-performance-analysis
...
1. If a Databricks-backed command includes a remediation object with `"next_action":"verify_shell_auth_then_retry_with_user_consent"`, follow that remediation instead of generic CLI/auth troubleshooting.
...
2. **First-run setup** (only when validate exits `60`, `61`, or `62`).
   ...
   **Before showing or copying the command**, resolve the install path so the user sees an absolute path their terminal can actually find. Run:
   ...
   - macOS: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/store-performance-analysis/asa.sh setup --skill store-performance-analysis" | pbcopy`
   - Windows: `echo bash ${CLAUDE_PLUGIN_ROOT}/skills/store-performance-analysis/asa.sh setup --skill store-performance-analysis | clip`
   - Linux: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/store-performance-analysis/asa.sh setup --skill store-performance-analysis" | xclip -selection clipboard 2>/dev/null || echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/store-performance-analysis/asa.sh setup --skill store-performance-analysis" | xsel --clipboard 2>/dev/null`
...
Stay within `{DATABASE}.{SCHEMA}`. If a documented table isn't found, report that honestly rather than hunting in other schemas.
...
2. Browse all tables in the schema (warehouse-specific):
   ...
   - BigQuery: `bq ls --max_results=10000 {DATABASE}:{SCHEMA}`
   - Databricks: `SHOW TABLES IN {DATABASE}.{SCHEMA};`
3. List datasets/databases (warehouse-specific):
   - BigQuery: `bq ls --max_results=10000 --project_id={DATABASE}`
   - Snowflake: `SHOW SCHEMAS IN DATABASE {DATABASE};`
   - Databricks: `SHOW SCHEMAS IN {DATABASE};`
...
5. Sample rows:
   - BigQuery: `bq head -n 5 {DATABASE}:{SCHEMA}.<table>`
   - Snowflake/Databricks: `SELECT * FROM {DATABASE}.{SCHEMA}.<table> LIMIT 5;`
