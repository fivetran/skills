#!/usr/bin/env python3
"""
asa.py — marketing-automation-analysis entry point.
...
def _config_dir() -> str:
    ...
    if os.path.isdir(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".fivetran", "skills", "marketing-automation-analysis")


def _profile_path() -> str:
    if os.environ.get("MARKETING_AUTOMATION_ANALYSIS_PROFILE_PATH"):
        return os.environ["MARKETING_AUTOMATION_ANALYSIS_PROFILE_PATH"]
...
def cmd_setup(
    ...
    if len(required_found) < min_required:
        _agent_print(
            ...
            "No supported marketing automation connector found on this destination. Return to your Claude Code chat for details.",
        )
...
def cmd_resolve(family: str, refresh_on_miss: bool) -> int:
    ...
    if refresh_on_miss and _resolve_credentials():
        ...
        skill_id = raw.get("skill", {}).get("id", "marketing-automation-analysis")
        cmd_setup(
...
# Per-table date column overrides. All other marketo__ tables default to created_timestamp.
_DATE_COL_MAP = {
    "marketo__lead_history": "date_day",
    "marketo__email_sends":  "activity_timestamp",
}
_DEFAULT_DATE_COL = "created_timestamp"
...
def main() -> int:
    ...
    if subcmd == "setup":
        ...
        skill_id = "marketing-automation-analysis"
        refresh  = False
        i = 0
        while i < len(rest):
            arg = rest[i]
