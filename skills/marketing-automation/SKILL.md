...
This skill uses a local profile at `~/.fivetran/skills/marketing-automation/profile.json`
to remember warehouse and connector preferences across sessions. First run creates it;
subsequent runs reuse it.

1. **Validate the local profile.**
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh validate
   ```
...
2. **First-run setup** (only when validate exits `60`, `61`, or `62`).
   ...
   **Before showing or copying the command**, resolve the install path so the user sees an absolute path their terminal can actually find. Run:
   ```bash
   echo "$CLAUDE_PLUGIN_ROOT/skills/marketing-automation/asa.sh"
   ```
   Use that absolute path in the command you show the user. Example block to present:

   > To finish setup, open a terminal and run:
   > ```
   > bash <resolved-absolute-path-to-asa.sh> setup --skill marketing-automation
   > ```
   ...
   - macOS: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh setup --skill marketing-automation" | pbcopy`
   - Windows: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh setup --skill marketing-automation" | clip`
   - Linux: `echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh setup --skill marketing-automation" | xclip -selection clipboard 2>/dev/null || echo "bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh setup --skill marketing-automation" | xsel --clipboard 2>/dev/null`
   ...
   - `validate` still returns `60` → **silently run setup yourself** and present the result naturally:
     ```bash
     bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh setup --skill marketing-automation 2>&1; echo "EXIT:$?"
     ```
...
3. **Resolve connector context.**
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh resolve marketo
   ```
   ...
   **On `relation not found`:** retry with `--refresh-on-miss`:
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh resolve marketo --refresh-on-miss
   ```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh readiness
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh check-cli <bq|snowflake_cli|databricks_cli>
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/generate-dashboard.py \
...
| Error | Response |
|---|---|
| Warehouse connection failure | "Cannot connect to warehouse. Run `bash ${CLAUDE_PLUGIN_ROOT}/skills/marketing-automation/asa.sh check-cli <tool>` to diagnose auth." |
