#!/usr/bin/env python3
"""
asa.py — customer-support-analysis entry point.
...
def _config_dir() -> str:
    ...
    if os.path.isdir(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".fivetran", "skills", "customer-support-analysis")


def _profile_path() -> str:
    if os.environ.get("CUSTOMER_SUPPORT_ANALYSIS_PROFILE_PATH"):
        return os.environ["CUSTOMER_SUPPORT_ANALYSIS_PROFILE_PATH"]
...
# in skills/ad-performance-analysis/asa.py and skills/store-performance-analysis/asa.py.
...
def cmd_setup(
    ...
    # The connections API does not support server-side service filtering, so we filter
    # client-side to REQUIRED_POOL. succeeded_at is retained for connection picking.
    all_connections = []
    ...
    # required.  We stop as soon as every active connection this skill cares about has been
    # linked to a QDM; the early-exit check runs between batches.  A batch size of 4 keeps
    # concurrency bounded while avoiding the worst-case serial scan when connections have no
    # quickstart transformation.
    _TXFM_BATCH = 4
    skill_conn_ids = {c["connection_id"] for c in all_connections if c["active"]}
    covered_conn_ids: set = set()

    txfm_details: List[dict] = []
    for i in range(0, len(active_txfm_ids), _TXFM_BATCH):
        if skill_conn_ids and covered_conn_ids >= skill_conn_ids:
            break
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(batch)) as pool:
            for detail in pool.map(_fetch_txfm_detail, batch):
                ...
                conn_ids = set((detail.get("transformation_config") or {}).get("connection_ids") or [])
                covered_conn_ids |= (conn_ids & skill_conn_ids)
    ...
    for family in sorted(REQUIRED_POOL | RECOMMENDED_POOL):
        ...
        if family in connection_overrides:
            ...
            if len(active) == 1:
                ...
                # so customers running parallel QDMs (e.g. multi-brand Zendesk) aren't silently
                # routed to the wrong one. Sorted by succeeded_at so the most recent is first.
    ...
    if len(required_found) < min_required:
        _agent_print(
            ...
            "No supported customer support connector found on this destination. Return to your Claude Code chat for details.",
        )
...
def cmd_resolve(family: str, refresh_on_miss: bool) -> int:
    ...
    if refresh_on_miss and _resolve_credentials():
        ...
        skill_id = raw.get("skill", {}).get("id", "customer-support-analysis")
        cmd_setup(
...
# Per-table date column overrides. All other zendesk__ tables default to created_at.
_DATE_COL_MAP = {
...
def main() -> int:
    ...
    if subcmd == "setup":
        ...
        skill_id = "customer-support-analysis"
        refresh  = False
        i = 0
        while i < len(rest):
            arg = rest[i]
